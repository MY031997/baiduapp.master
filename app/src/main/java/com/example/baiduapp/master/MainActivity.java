package com.example.baiduapp.master;
...
public class MainActivity extends BaseActivity implements SensorEventListener, View.OnClickListener {
    ...
    public void getLocationClientOption() {
        ...
        client = new LocationClient(this);//定位初始化
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            //打卡范围
            mDestinationPoint = new LatLng(location.getLatitude() * 1.0001
                    , location.getLongitude() * 1.0001);//假设公司坐标
            setCircleOptions();
            //计算两点距离,单位：米
            mDistance = DistanceUtil.getDistance(mDestinationPoint, LocationPoint);
            if (mDistance <= DISTANCE) {
                //显示文字
                setTextOption(mDestinationPoint, "您已在公司范围内", "#7ED321");
                //目的地图标
                setMarkerOptions(mDestinationPoint, R.mipmap.arrive_icon);
                //按钮颜色
                commit_bt.setBackground(getResources().getDrawable(R.mipmap.restaurant_btbg_yellow));
                mBaiduMap.setMyLocationEnabled(false);
            } else {
                setTextOption(LocationPoint, "您不在公司范围之内", "#FF6C6C");
                setMarkerOptions(mDestinationPoint, R.mipmap.restaurant_icon);
                commit_bt.setBackground(getResources().getDrawable(R.mipmap.restaurant_btbg_gray));
                mBaiduMap.setMyLocationEnabled(true);
            }
            mDistance_tv.setText("距离目的地_" + mDistance + "米");
            //缩放地图
